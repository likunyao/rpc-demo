...
class MyChannel : public ::google::protobuf::RpcChannel {
   public:
    ...
    virtual void CallMethod(
        ...
        serialzied_str.insert(
            0, std::string((const char *)&serialzied_size, sizeof(int)));
...
void MyServer::start(const std::string &ip, const int port) {
    ...
    while (true) {
        ...
        std::cout << "recv from client:" << sock->remote_endpoint().address()
                  << std::endl;
        ...
        //
        dispatch_msg(meta.service_name(), meta.method_name(),
                     std::string(&data[0], data.size()), sock);
    }
}
...
void MyServer::dispatch_msg(
    ...
    auto done = ::google::protobuf::NewCallback(
        this, &MyServer::on_resp_msg_filled, recv_msg, resp_msg, sock);
